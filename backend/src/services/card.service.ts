 ...
 */
export const searchCards = async (q: string, limit = 20) => {
...
export const moveCard = async (
  ...
  if (!prevOrder && !nextOrder) {
    const destinationHasCards = await Card.exists({ listId, _id: { $ne: cardId } });
  ...
  const card = await Card.findByIdAndUpdate(
    cardId,
    { $set: { listId, order }, $inc: { version: 1 } },
    { new: true }
  );
